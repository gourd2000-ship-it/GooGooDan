...
- [x] M1 기반의 서버 단위 테스트(33개), 클라이언트 단위 테스트(14개), 클라이언트 프로덕션 빌드가 통과했다.
- [ ] M2 학생 로그인·보호 화면·프로필·실제 랭킹 흐름을 TDD로 완료한다.
- [ ] M3 관리자 학생 명부와 원자적 CSV 일괄 등록을 TDD로 완료한다.
- [ ] M4 관리자 성취 대시보드 필터·매트릭스를 TDD로 완료한다.
- [ ] M5 배포 설정 안내와 전체 테스트·빌드 검증을 완료한다.

## M2~M5 실행 순서

1. 학생 PIN 로그인 API 클라이언트와 키패드 UI를 연결하고, 보호 화면·로그아웃·PIN 변경을 완성한다.
2. 명예의 전당에서 예시 데이터를 제거하고 실제 서버 오류/빈 상태를 표시한다.
3. 관리자 인증 헤더를 쓰는 학생 명부 API와 CSV 원자적 등록을 구현한 뒤 관리자 UI를 연결한다.
4. 성취 API에 학년·반 필터를 추가하고, 학생×2~9단 대시보드를 구현한다.
5. 환경변수·마이그레이션·HTTPS 운영 절차를 문서화하고 모든 테스트와 빌드를 실행한다.

각 기능은 테스트를 먼저 실패시키고(RED), 최소 구현으로 통과시킨 뒤(GREEN), 전체 회귀 검증을 수행한다.
